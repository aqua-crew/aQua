...
class MeasuredLine {
    constructor($line, box, lineNum) {
        this.$code = $line.children[1].firstChild
        this.box = box
        this.lineNum = lineNum
        this.height = $line.getBoundingClientRect().height
    ...
    getInsideLogicalY(y) {
        ...
        for (; insideY >= 0; insideY--) {
            const bottom = this.transformToRealBottom(lineRects[insideY].bottom) - this.box.top // (lineHeight - fontSize) / 2
    ...
    getInsidePhysicalY(insideY) {
        const lineRects = this.getClientRects()

        return lineRects[insideY].bottom - this.box.top
    }
    ...
    genMap() {
        const $nodes = this.$code.childNodes
